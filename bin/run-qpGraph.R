...
plot_admixtree <- function(layout, ...) {
  ggraph(layout, color = "darkgray") + 
    ...
    scale_color_manual(values = subgroups_pal, guide = "none", na.value = "darkgray") +
...
fam <- read_fam(paste0(data_prefix, ".fam")) %>%
  ...
  filter(
    age == "Contemporan",
    group == "NAT" | pop %in% c("Mbuti", "Han")
  )

## Define Models --------
node <- tibble(
  name = c("Root", "SA1", "SA2", "SA3", "Out", "WestAndes", "Amazon", "South", "SouthEast"),
  region = name
)
models <- list(
  model1 = tribble(
    ...
    "SA2", "WestAndes", "edge",
    "SA2", "Amazon", "edge",
    "Amazon", "SouthEast", "admix",
    "WestAndes", "SouthEast", "admix",
  ),
  model2 = tribble(
    ~ from, ~ to, ~type,
    "Root", "Out", "edge",
...
if (!file.exists(model_dat_file)) {
  ...
  if (!dir.exists(data_f2_dir)) {
    extract_f2(data_prefix, data_f2_dir, pops = pop_subset)
  }
...
model_f4 <- dat %>%
  mutate(Out = parse_factor(Out, c("Mbuti", "Han", "Mayan", "Pima"))) %>%
  mutate(score = map_dbl(graph, "worst_residual")) %>%
